...
import HeaderComponent from '../compartidos/HeaderComponent';
import Loading from '../Loading'

const DetallePacientesComponent = ({ route }) => {
	/**
	 * estoy pensando que para que funcione deberia hacer el join afuera del detalle y que aca le llegue ya el paciente.
	 * De hecho el join estaria bueno que este en querys ya que va a ser llamado de muchos lados distintos.
	 */
	...
	const [pacienteJoin, setpacienteJoin] = useState(null)
...
/* 	useEffect(
		() => {
			...
			//paciente.edad = 0;
			joins();
			// setpacienteJoin({...paciente,edad:0})
		},
		[ paciente ]
	);
	...
	console.log("joins")
		if (db != null) {
			setpacienteJoin(null);
			await db.transaction(async(tx) => {
			console.log("joins2 con ",paciente.numeroHC, paciente.idHospital)
			 await	tx.executeSql(
					...
					[ paciente.numeroHC, paciente.idHospital ],
					(tx, results) => {
						console.log("joins3")
	...
	const sqlQueryInsertIntoSignosVitales = () => {
		{
			if (
				lecturaFrecuenciaRespiratoria == '' ||
				lecturaSaturacionOxigeno == '' ||
				lecturaSaturacionOxigenoEpoc == '' ||
				lecturaOxigenoSuplementario == '' ||
				lecturaPresionSistolitica == '' ||
				...
				db.transaction(
					(tx) => {
						...
						tx.executeSql(
							...
							(tx, results) => {},
							(tx, err) => {
								console.log('Error al insertar en signos vitales ' + err);
							}
						);
					},
					(err) => {
	...
	const calcularValor = (updatePaciente) => {
		...
		llega3 = true;
		if (updatePaciente.frec_resp <= 11 && updatePaciente.frec_resp > 8) valor += 1;
		if (updatePaciente.frec_resp <= 24 && updatePaciente.frec_resp > 20) valor += 2;
		if (updatePaciente.frec_resp > 24) valor += 3;
		llega3 = true;
		if (updatePaciente.sat_oxi <= 91) valor += 3;
		llega3 = true;
		if (updatePaciente.sat_oxi >= 92 && updatePaciente.sat_oxi <= 93) valor += 2;
		if (updatePaciente.sat_oxi >= 94 && updatePaciente.sat_oxi <= 95) valor += 1;
		//oxigeno Epoc
		if (updatePaciente.sat_epoc <= 83) valor += 3;
		llega3 = true;
		if (updatePaciente.sat_epoc >= 84 && updatePaciente.sat_epoc <= 86) valor += 2;
		if (updatePaciente.sat_epoc >= 87 && updatePaciente.sat_epoc <= 88) valor += 1;
		...
		llega3 = true;
		if (updatePaciente.frec_card >= 41 && updatePaciente.frec_card <= 50) valor += 1;
		if (updatePaciente.frec_card >= 91 && updatePaciente.frec_card <= 110) valor += 1;
		if (updatePaciente.frec_card >= 111 && updatePaciente.frec_card <= 130) valor += 2;
		if (updatePaciente.frec_card > 131) valor += 3;
		llega3 = true;
		if (updatePaciente.temp <= 35) valor += 3;
		llega3 = true;
		if (updatePaciente.temp >= 35 && updatePaciente.temp <= 35.5) valor += 1;
		if (updatePaciente.temp >= 38 && updatePaciente.temp <= 39) valor += 1;
		if (updatePaciente.temp > 39) valor += 2;
		if (updatePaciente.dimeroD > 1000) valor += 1;
		if (updatePaciente.linfopenia < 1000 && updatePaciente.linfopenia >= 500) valor += 1;
		...
		setpacienteJoin(updatePaciente);
   ...
   */
	...
	if (!visibleCargar) {
		if (paciente != null) {
					...
					<Content style={styles.container}>
						<ScrollView style={styles.container}>
							<View style={styles.item}>
								...
								<Text>
									Urgencia: {paciente.calificacion} ({paciente.gravedad})
								</Text>
						...
						<View style={styles.button}>
							<Button onPress={openCargarDatos}>
			...
			return (
				<View>
					...
					{/* <Button title="Volver" onPress={() => props.navigation.navigate('Pacientes')} /> */}
				</View>
			);
		}
	} else {
		return (
			<Container animationType="slide" style={styles.container}>
				<HeaderComponent
					title={'Cargar Datos Clinicos'}
					subTittle={paciente.nombre + ' ' + paciente.apellido}
				/>
            ...
            /> */}
				...
				<View style={styles.internalInput}>
					<View style={styles.button}>
						<Button onPress={() => TextInputHandler()}>
							<Text>Cargar</Text>
						</Button>
					</View>
				</View>
