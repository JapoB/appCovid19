...
export const QueryInicial = async () => {
	db.transaction((tx) => {
	...
	db.transaction((tx) => {
		...
		tx.executeSql('DROP TABLE Paciente', [], () => console.log('Dropeo Paciente exitoso'));
	});

	db.transaction((tx) => {
		console.log('Dropeo la tabla Alerta');
		tx.executeSql('DROP TABLE Alerta', [], () => console.log('Dropeo Alerta exitoso'));
	});

	db.transaction((tx) => {
		console.log('Dropeo la tabla Cierre Episodio');
		tx.executeSql('DROP TABLE CierreEpisodio', [], () => console.log('Dropeo Cierre Episodio exitoso'));
	});

	db.transaction((tx) => {
		console.log('Dropeo la tabla usuarios ');
		tx.executeSql('DROP TABLE Usuario', [], () => console.log('Dropeo usuarios exitoso'));
	});

	db.transaction((tx) => {
		console.log('Dropeo la tabla usuarios Hospital ');
		tx.executeSql('DROP TABLE UsuarioHospital', [], () => console.log('Dropeo usuarios Hospital exitoso'));
	});

	db.transaction((tx) => {
		console.log('Dropeo la tabla Isla ');
		tx.executeSql('DROP TABLE Isla', [], () => console.log('Dropeo Isla exitoso'));
	});

	db.transaction((tx) => {
		console.log('Dropeo la tabla usuario Sector ');
		tx.executeSql('DROP TABLE UsuarioSector', [], () => console.log('Dropeo UsuarioSector  exitoso'));
	});

	db.transaction((tx) => {
		console.log('Dropeo la tabla Sectores ');
		tx.executeSql('DROP TABLE Sector', [], () => console.log('Dropeo Sectores exitoso'));
	});

	db.transaction((tx) => {
		console.log('Dropeo la tabla Cama ');
		tx.executeSql('DROP TABLE Cama', [], () => console.log('Dropeo Cama exitoso'));
	});

	db.transaction((tx) => {
		console.log('Dropeo la tabla Laboratorio  ');
		tx.executeSql('DROP TABLE Laboratorio', [], () => console.log('Dropeo Laboratorio  exitoso'));
	});

	db.transaction((tx) => {
		console.log('Dropeo la tabla RXTORAX  ');
		tx.executeSql('DROP TABLE RxTorax', [], () => console.log('Dropeo Paciente RXTORAX '));
	});
	...
	await db.transaction(
		(tx) => {
			...
			tx.executeSql(
				'CREATE TABLE Hospital (id INT PRIMARY KEY NOT NULL, nombre varchar(40),' +
						...
						() => {
							db.transaction(
								(tx) => {
									...
									var query =
										'INSERT INTO Usuario (cuil,clave, email,telefono) VALUES' +
								...
								() => {
									db.transaction(
										(tx) => {
											...
											tx.executeSql(
												'CREATE TABLE Paciente(dni INT NOT NULL, tipoDocumento varchar(10) NOT NULL,numeroHC varchar(20) NOT NULL,' +
										...
										() => {
											db.transaction(
												(tx) => {
													...
													tx.executeSql(
														'INSERT INTO Hospital (id, nombre,calle,numero,CP,planoCamas) VALUES (1,"Castro","Bs As","400","8300","Este es el plano")',
														[],
														(tx, results) => {
														...
														() => {
															db.transaction(
																(tx) => {
																	...
																	tx.executeSql(
																		'Insert into Paciente (dni, tipoDocumento,numeroHC,nombre,apellido,genero,' +
																		...
																		() => {
																			console.log(
																				'Insercion en pacientes exitosa'
																			);
																		},
																		(tx, err) => {
																			console.log(err);
																	...
																	db.transaction(
																		(tx) => {
																			tx.executeSql(
																				...
																				() => {
																					console.log(
																						'Creacion de tabla CierreEpisodio exitosa'
																					);
																				}
																			);
																		},
																			...
																			db.transaction(
																				(tx) => {
																					...
																					tx.executeSql(
																						...
																						() => {
																							console.log(
																								'Creacion de tabla alertas exitosa'
																							);
																						}
																					);
																				},
																					...
																					db.transaction(
																						(tx) => {
																							...
																							tx.executeSql(
																								...
																								() => {
																									console.log(
																										'Creacion de tabla UsuarioSector exitosa'
																									);
																								}
																							);
																						},
																							...
																							db.transaction(
																								(tx) => {
																									...
																									tx.executeSql(
																										...
																										() => {
																											console.log(
																												'Creacion de tabla Isla exitosa'
																											);
																										}
																									);
																								},
																									...
																									db.transaction(
																										(tx) => {
																											...
																											tx.executeSql(
																												...
																												() => {
																													console.log(
																														'Creacion de tabla Sector exitosa'
																													);
																												}
																											);
																										},
																											...
																											db.transaction(
																												(
																													...
																													tx.executeSql(
																														...
																														() => {
																															console.log(
																																'Creacion de tabla Cama exitosa'
																															);
																														}
																													);
																												},
																													...
																													db.transaction(
																														(
																															...
																															tx.executeSql(
																																...
																																() => {
																																	console.log(
																																		'Creacion de tabla Laboratorio exitosa'
																																	);
																																}
																															);
																														},
																															...
																															db.transaction(
																																(
																																	...
																																	tx.executeSql(
																																		...
																																		() => {
																																			console.log(
																																				'Creacion de tabla RxTorax exitosa'
																																			);
																																		}
																																	);
																																},
																																...
																																() => {
																																	db.transaction(
																																		(
																																			...
																																			console.log(
																																				'Creacion de SignosVitales'
																																			);
																																			var query =
																																				'CREATE TABLE IF NOT EXISTS SignosVitales (id_hospital INTEGER, numeroHC varchar(20), fecha date, fec_resp REAL, sat_oxi REAL, sat_epoc REAL,presSist REAL, frec_card REAL, temp REAL,nivelConciencia varchar(15), auditoria VARCHAR(20),PRIMARY KEY (id_hospital,numeroHC,fecha),FOREIGN KEY (auditoria) REFERENCES Usuario(cuil), FOREIGN KEY (id_hospital) REFERENCES Hospital (id),FOREIGN KEY (numeroHC) REFERENCES paciente (numeroHC))';
																																			var params = [];
																																			tx.executeSql(
																																				query,
																																				params,
																																				...
																																				(
																																					...
																																					console.log(
																																						'Tabla signos vitales no pudo ser creada'
																																					);
																																				}
																																			);
																																		},
																																		...
																																		() => {
																																			db.transaction(
																																				(
																																					...
																																					var query =
																																						'INSERT INTO ALERTA (idHospital, numeroHC, fecha, gravedad, calificacionEnfermero,calificacionMedico,auditoriaEnfermero,auditoriaMedico) VALUES ' +
																																				...
																																				() => {
																																					db.transaction(
																																						(
																																							...
																																							var query =
																																								'INSERT INTO Isla (idIsla,idHospital,idLider) VALUES ' +
																																								"('1',1,1)";
																																							var params = [];
																																							tx.executeSql(
																																								query,
																																						...
																																						() => {
																																							db.transaction(
																																								(
																																									...
																																									console.log(
																																										'insertar de SignosVitales'
																																									);
																																									var query =
																																										'INSERT INTO SignosVitales (id_hospital,numeroHc,fecha,fec_resp,sat_oxi,sat_epoc,presSist,frec_card,temp,nivelConciencia,auditoria) VALUES ' +
																																									...
																																									tx.executeSql(
																																										...
																																										(
																																											...
																																											console.log(
																																												'Insertar en Signos vitales exitoso'
																																											);
																																										},
																																										(
																																											tx,
																																											...
																																											console.log(
																																												'error en insertar signos vitales'
																																											);
																																											console.log(
																																												err
																																											);
																																								...
																																								() => {
																																									db.transaction(
																																										(
																																											...
																																											var query =
																																												'INSERT INTO Sector(idIsla,idHospital,idSector) VALUES ' +
																																												"('1',1,1)";
																																											var params = [];
																																											tx.executeSql(
																																												query,
																																										...
																																										() => {
																																											db.transaction(
																																												(
																																													...
																																													var query =
																																														'INSERT INTO Cama (idIsla,idHospital,idSector,idCama,estado,ubicacionX,ubicacionY,orientacion,numeroHC) VALUES ' +
																																												...
																																												() => {
																																													db.transaction(
																																														(
																																															...
																																															console.log(
																																																'insertar de Laboratorio'
																																															);
																																															var query =
																																																'INSERT INTO Laboratorio (numeroHC,idHospital,fecha,cuil,dimeroD,linfopenia,proteinaC) VALUES ' +
																																															...
																																															tx.executeSql(
																																																...
																																																(
																																																	...
																																																	console.log(
																																																		'Insertar en laboratorio exitoso'
																																																	);
																																																},
																																																(
																																																	tx,
																																																	...
																																																	console.log(
																																																		'error al insertar en laboratorio'
																																																	);
																																																}
																																															);
																																														},
